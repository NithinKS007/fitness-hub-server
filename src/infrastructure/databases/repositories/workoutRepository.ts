...
import { Workout, WorkoutChartData } from "../../../domain/entities/workoutEntity";
...
export class MongoWorkoutRepository implements WorkoutRepository {
  public async addWorkout(data: WorkoutDBdto[]): Promise<Workout[]>{
    const result =  await workoutModel.insertMany(data)
    return result
  }
  ...
  public async getWorkoutsByUserId(userId:IdDTO,data:GetWorkoutQueryDTO):Promise<{workoutList:Workout[],  paginationData:PaginationDTO}> {
    ...
    if(data){
      ...
      const workoutList = await workoutModel.find(matchQuery)
      .sort({ date: 1 }) 
      .skip(skip)
      .limit(limitNumber)
      .lean() 
  ...
  public async markAsCompleted(setId: IdDTO): Promise<Workout | null> {
    ...
    const updatedSet = await workoutModel.findOneAndUpdate(
      { _id: objectId },
  ...
  public async getUserDashBoardChartData(data: CustomUserDashBoardQueryDTO): Promise<WorkoutChartData[]> {
      
    const {userId,bodyPart,startDate,endDate} = data
  ...
  public async getTodaysTotalPendingWorkouts(userId:string,startDate:Date,endDate:Date): Promise<number> {
    const result = await workoutModel.aggregate([
      {
        $match:{
          userId: new mongoose.Types.ObjectId(userId),
    ...
    return result[0]?.todayPendingWorkouts ? result[0]?.todayPendingWorkouts :0
  }

  public async getTodaysTotalCompletedWorkouts(userId:string,startDate:Date,endDate:Date): Promise<number> {
    const result = await workoutModel.aggregate([
      {
        $match:{
          userId: new mongoose.Types.ObjectId(userId),
    ...
    return result[0]?.todayCompletedWorkouts ? result[0]?.todayCompletedWorkouts :0
  }
}
