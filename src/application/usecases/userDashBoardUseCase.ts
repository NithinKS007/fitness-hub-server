...
export class UserDashBoardUseCase {
    ...
    public async getUserDashBoardData(data:UserDashBoardQueryDTO):Promise<UserDashBoard>{
        const { period,bodyPart,userId } = data
    ...
    private async getTodaysTotalPendingWorkouts(userId:string):Promise<number> {
        ...
        return await this.workoutRepository.getTodaysTotalPendingWorkouts(userId,startDate,endDate)
    }
    private async getTodaysTotalCompletedWorkouts(userId:string):Promise<number> {
        const {startDate,endDate} = getDateRange("Today")
        return await this.workoutRepository.getTodaysTotalCompletedWorkouts(userId,startDate,endDate)
    }

    private async getUserChartData({ period,bodyPart,userId }:{period:string,bodyPart:string,userId:string}):Promise<WorkoutChartData[]>{
        const {startDate,endDate} = getDateRange(period)
        const chartData = await this.workoutRepository.getUserDashBoardChartData({startDate:startDate,endDate:endDate,userId:userId,bodyPart:bodyPart})
        return chartData
    }
}
